...
$$
Such a system has four stable equilibria at the corners of a 2 x 2 square (i.e, $(\pm 1, \pm 1)$), and an unstable one at $(0,0)$.

We build the flow function, not forgetting to write the input as a vector:
...
By plotting the field $V$, we can see the potential:
...
First, we define the parameters' values and the flow function:
...
Secondly, we need to define our region of interest:
...
filled.contour(xs, ys, result$V, 
        ...
        levels = seq(0, 8, by = 0.1),
        main = 'Approximate potential', xlab = 'x', ylab = 'y')

```

Being this not a gradient system it is advisable to plot the estimated error. The areas in green represent small approximation error, so the previously obtained potential can be safely used in those regions.
