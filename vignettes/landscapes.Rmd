...
\begin{equation}
\boxed{ 
  V(\vec{x_0} + \Delta \vec x) \approx  V(\vec{x_0}) - \vec f(\vec x_0) \cdot \Delta \vec x - \frac{1}{2} \Delta \vec x^T  J_{Symm} (\vec{x_0})  \Delta \vec x
}
(\#eq:Potential)
\end{equation}

Using this potential $V(\vec x)$, we can rewrite our dynamical system as in equation \@ref(eq:anyD). Due to the approximations used, this result will only be true on the vicinity of $\vec{x_0}$.

The stability landscape described in \@ref(eq:Potential) not only has been derived from a linearized system, but also completely neglects the effects of the curl part of the system As with any other approximation we need a way to estimate its error. From equation \@ref(eq:FieldDecomposition) it is apparent that we can use the euclidean size of $\vec f_{curl} (\vec x)$ as an approximation of the local error introduced by our algorithm. The relative error can be approximated using equation \@ref(eq:error).
...
\begin{equation}
\Delta V(\vec{x_1}, \vec{x_0}) \approx - \vec f(\vec x_0) \cdot (\vec{x_1} - \vec{x_0}) - \frac{1}{2} (\vec{x_1} - \vec{x_0})^T  J_{Symm} (\vec{x_0})  (\vec{x_1} - \vec{x_0})
(\#eq:PotentialNum)
\end{equation}
