...
(ref:penrose) The Penrose stair, a classical example of an impossible object. In this surface, it is possible to walk in a closed loop while permanently going downhill. This kind of surfaces were popularized by the Dutch artist M.C. Escher (for a beautiful example, see @escher. The same idea is explored in @escher1961).
...
The more fundamental error due to ignoring the curl component of our system cannot be affected by the grid's step choice. From equation \@ref(eq:FieldDecomposition) it is apparent that we can use the euclidean size of $\vec f_{curl} (\vec x)$ as an approximation of the local error introduced by neglecting the curl in our algorithm. The relative error due to this effect can be estimated using equation \@ref(eq:error).
