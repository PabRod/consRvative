...
(ref:escher) Detail of M.C. Escher's _Klimmen en dalen_. Lithography, 1960. In this surface, it is possible to walk in a closed loop while permanently going downhill. Unfortunately, such a surface only exists in the imagination of surrealist artists
...
$\vec f_{grad} (\vec x)$ captures the part of the system that can be associated to a potential function, while $\vec f_{curl} (\vec x)$ represents the deviation from this ideal case. We'll use $\vec f_{gradient} (\vec x)$ to compute an approximate potential. The absolute error of this approach will be given by the euclidean size of the curl term $\mid \vec f_{curl} (\vec x) \mid$. In those regions where the gradient term is stronger than the curl term, the condition \@ref(eq:anyDcond) will be approximately fulfilled, and thus the previously calculated potential will represent an acceptable approximation of the underlying dynamics. The next steps show an easy way of achieving such a decomposition.
...
The gradient term can thus be associated to a potential $V(\vec x)$ that can be computed analytically using a line integral (see equation \@ref(eq:anyDint)). The result of this integration yields an analytical expression for the potential difference between the reference point $\vec{x_0}$ and another point $\vec{x_1} \equiv \vec{x_0} + \Delta \vec{x}$ separated by a distance $\Delta \vec x$ (see equation \@ref(eq:Potential)).
...
1. It completely neglects the effects of the curl part of the system
2. It has been derived from a linearized system

From equation \@ref(eq:FieldDecomposition) it is apparent that we can use the euclidean size of $\vec f_{curl} (\vec x)$ as an approximation of the local error introduced by neglecting the curl in our algorithm. The relative error due to this effect can be estimated using equation \@ref(eq:error).

\begin{equation}
\boxed{
  err(\vec x) \approx \frac{\mid \vec f_{curl}(\vec x) \mid}{\mid \vec f_{grad}(\vec x) \mid + \mid \vec f_{curl}(\vec x) \mid} = \frac{\mid J_{Skew}(\vec{x}) \mid}{\mid J_{Symm}(\vec{x}) \mid + \mid J_{Skew}(\vec{x}) \mid}
}
(\#eq:error)
\end{equation}

The error due to linearization is roughly proportional to $\mid \Delta \vec{x} \mid^3$ in equation \@ref(eq:Potential), being $\mid \Delta \vec{x} \mid$ the euclidean distance to the reference point. This error can be minimized by using \@ref(eq:Potential) as a recurrence relation, that is, by iterating it in small steps in space. A working algorithm is described in section \@ref(algorithm).

## Numerical algorithm {#algorithm}
...
The abovementioned ideas can be applied to the numerical computation of the approximate potential over a grid of points by following this list of steps (see also sections \@ref(sec:detail) and \@ref(sec:2d-example) in the online appendix):

1. Create a grid in the phase plane $\left\lbrace \vec x_i \right\rbrace$
2. Calculate the jacobian $J(\vec{x_i})$ in each grid point
3. Decompose each jacobian in its skew $J_{Skew}(\vec{x_i})$ and symmetric $J_{Symm}(\vec{x_i})$ components
4. Assign potential 0 to the first point in the grid
5. Use equation \@ref(eq:Potential) to iterativelly calculate the values of the potential at each point of the grid
6. Use equation \@ref(eq:error) as a measure of the quality of the approximation point by point.

By introducing a grid, we expect the linearization error to decrease with the grid's step size. The more fundamental error due to ignoring the curl component of our system cannot be affected by the grid's step choice, but the algorithm provides us with a measure of its magnitude point-by-point.
...
Generalizing the previous example we see that, for a generic point $(x_i, y_j)$, we can compute the approximate potential using the closed formula \@ref(eq:NumericalRecipe). Formula \@ref(eq:NumericalRecipe) has been derived sweeping first in the horizontal direction and next in the vertical one. Of course, we can choose different paths of summation. Because we are building our potential neglecting the curl part of our vector field, we know that our results will converge to the same solution regardless of the chosen path.
...
By direct application of equation \@ref(eq:error) we can easily find the relative error at each point of our grid (equation \@ref(eq:2Derror)).

\begin{equation}
\boxed{
err(x_i, y_j) = \frac{\mid J_{Skew}(x_i, y_j)\mid}{\mid J_{Symm}(x_i, y_j)\mid + \mid J_{Skew}(x_i, y_j)\mid}
}
(\#eq:2Derror)
\end{equation}
