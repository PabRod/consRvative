...
Dynamics in equation $\ref{eq:2D}$ and the condition for the crossed derivatives $\ref{eq:2Dcond}$ can be straightforwardly generalized (see equations $\ref{eq:anyD}$ and $\ref{eq:anyDcond}$) to systems with an arbitrary number of state variables $\vec x = (x_1, ..., x_n)$. Particularly,
...
then there is a surface $V(\vec x)$ verifying:
...
where the line integral in $\ref{eq:anyDint}$ is computed along the curve $\Gamma$ joining the points $\vec x_0$ and $\vec x$.
...
This represents a natural decomposition of the field $\vec f(\vec x)$ in a gradient and a non-gradient term. More specifically, the only non-gradient term is the one containing the skew part of the jacobian\footnote{This is deeply related with the Helmholtz decomposition: it is proven that, for a huge range of easy to fulfil conditions, any field can be decomposed as $\vec f (\vec x) = - \vec \nabla V (\vec x) + \vec \nabla \times \vec A (\vec x )$, where $V$ and $\vec A$ are known as scalar and vector potentials. Both of them have a straightforward natural interpretation, and are widely used, for instance, in electromagnetism and fluid dynamics}. The gradient term $\vec f(\vec{x_0}) + J_{Symm}(\vec{x_0}) \cdot \Delta \vec x$ can thus be associated to a (pseudo)stability landscape (see equation $\ref{eq:TaylorExpDecomposedAndGradient}$).
