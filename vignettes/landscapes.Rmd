...
Such a surface $V(x,y)$ exists (and thus can be computed) if and only if the crossed derivatives of the elements of the flow are equal (equation $\ref{eq:2Dcond}$). Flows satisfying equation $\ref{eq:2Dcond}$ are known as conservative, irrotational or gradient flows. These flows have an important property: the value of a line integral inside such a flow only depends on the starting and ending point of the integration curve, and not in its shape (cf. section 8.3 of @Marsden2003).
...
It is important to note that equation $\ref{eq:2Dcond}$ is an equality of functions, meaning that both of them are point-to-point identical. If an only if condition $\ref{eq:2Dcond}$ holds, we can use a line integral (@Marsden2003, section 7.2) to invert $\ref{eq:2D}$ and calculate $V(x,y)$ using the flow equations $f(x,y)$ and $g(x,y)$ as an input. An example of this inversion is equation $\ref{eq:2Dint}$, where we have chosen an integration path composed of a horizontal and a vertical line.
...
The algorithm we present is an attempt to preserve as much as possible from the classical potential theory (indeed, it reconstructs the classical potential when it does exist). The price we pay is to calculate a potential that, in general, is only locally meaningful. The estimated error lets us know where does this happen. Our claim that our algorithm can be a contribution to the field rests on three pillars:

- It is simple. The mathematical background required is covered by any introductory course in vector calculus. Additionally, we provide the algorithm in a ready to use _R_ package.
- It is reliable. At each step the local error is computed and stored, providing, so to say, a map of which part of our stability landscape are dangerous to visit.
- It is fast. The rendering of a printing quality surface can be performed in no more than a few minutes in a personal laptop.

After pointing out all the difficulties of calculating potentials for higher dimensional systems, maybe it is worth to ask ourselves the question: do we really need them? The concept of potential is a cornerstone of physics. We know that the time evolution of several physical systems is governed by a potential function. In physics, potentials can be related with measurable concepts like energy, so its use goes way further than visualization. Generic stability landscapes lack this straightforward link with measurable quantities, and in most cases are just no more (and no less) than a visual aid. It may be worth reconsidering why do we prefer the idea of stability landscape over the good-old phase plane figure. It is true that the latter is slightly less intuitive than the stability landscape, but it has a very desirable property: existence under general circumstances.
...
# Some loose ideas
Here I write some ideas that may or may be not part of the final manuscript, but I consider worth keeping in mind.

## Why the existence of a potential fails for more than 1 dimension? Intuitive approaches
